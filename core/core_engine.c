...
static void engine_load_plugin(const char* full_path) {
    ...
    if (new_node == NULL) {
        ui_log("[SYS Error]: Memory allocation failed\n");
        ui_cleanup();
        exit(EXIT_FAILURE);
    }
...
static void engine_run_all() {
    ...
    while (curr != NULL) {
        curr->curr_plugin->run(mon_win, plugin_log_win);
        curr = curr->next;
    }
    ...
    while (i < length) {
        ...
        if (event->len) {
            ...
            if (is_create) {
                ...
                if (!is_valid_file) {
                    ui_log("[Core Warning]: This folder is for .so files only, deleting %s\n", event->name);
...
void engine_load_existing_plugins(const char* path) {
    ...
    if (dir == NULL) {
        ui_log("[SYS Error]: Couldn't open dir '%s'\n", path);
        return;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_name[0] != '.') {
            ...
            if (!is_valid_file) {
                ui_log("[Core Warning]: This folder is for .so files only, deleting this file\n");
                remove(fullPluginPath);
            }
            engine_load_plugin(fullPluginPath);
        }
...
void engine_start_monitor(const char* path) {
    ...
    if (fd < 0) {
        ui_log("[SYS Error]: Couldn't init inotify\n");
        ui_cleanup();
        exit(EXIT_FAILURE);
    }
    /*
    ...
    */ 
    ...
    ui_log("[SYS Info]: Ready to listen to events\n");
